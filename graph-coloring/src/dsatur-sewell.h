...
namespace dsaturSewell {
    ...
    int sewellRule(int vertexIndex, grafo::Grafo& G, vector<int> & cores, set<int> & uncolored, int k, fstream& logStream) {
        debug::debug("{action: 'begin_sewell', value: " + to_string(vertexIndex) + "}", logStream);
        ...
        debug::debug("{action: 'end_sewell', key: " + to_string(vertexIndex) + ", value: " + to_string(sum) +  "}", logStream);
        return sum;
    }
    ...
    int reordenarProximoIndice(vector<int>& ordenacao, int i, grafo::Grafo& G, vector<int> vCores, int k, fstream& logStream) {
        ...
        debug::debug("{action: 'get_uncolored', value: " + to_string(uncolored.size()) + " }", logStream);
