...
namespace dsaturPass {
    ...
    int passRule(int vertexIndex, grafo::Grafo& G, vector<int> & cores, set<int> & tied, int k, fstream& logStream) {
        debug::debug("{action: 'begin_pass', value: " + to_string(vertexIndex) + "}", logStream);
        ...
        debug::debug("{action: 'get_available_colors', key: " + to_string(vertexIndex) + ", value: " + vectorUtils::serializarSet(coresDisponiveis) + " }", logStream);
        ...
        for (int pos: G.listaAdj[vertexIndex]) {
            ...
            debug::debug("{action: 'get_available_colors', key: " + to_string(pos) + ", value: " + vectorUtils::serializarSet(coresDisponiveisVizinho) + " }", logStream);
            ...
            debug::debug("{action: 'get_intersection', value: " + to_string(intersection.size()) + " }", logStream);
        ...
        debug::debug("{action: 'end_pass', key: " + to_string(vertexIndex) + ", value: " + to_string(sum) +  "}", logStream);
        return sum;
    }
    ...
    int reordenarProximoIndice(vector<int>& ordenacao, int i, grafo::Grafo& G, vector<int> vCores, int k, fstream& logStream) {
        ...
        set<int> tied;
        debug::debug("{action: 'get_tied', value: " + to_string(tied.size()) + " }", logStream);
        ...
        for (int prox = i+1; prox < G.n; prox++) {
            ...
            if (proxSat > vencedorSat) {
                ...
                debug::debug("{action: 'tie' , key: " + to_string(ordenacao[prox]) + ", value: " + to_string(ordenacao[indiceVencedor]) + "}", logStream);
    ...
    vector<int> dsaturPass (grafo::Grafo& G, fstream& logStream, int & backtrackingVertices) {
        ...
        dsaturPass::reordenarProximoIndice(ordenacao, i, G, coloracaoAtual, k, logStream);            
